...
# what tags will we be looking for in LOGIN_FILE
TAGS = [
    "addons_path",
    "admin_passwd",
    "create_database",
...
def collect_existing(file_name, tags_keep):
    """collect values from the existing config file

    Args:
        file_name (string): path to the file to read values from
        tags_keep (list of strings): tags to read from existing cfg
    Return:
        dict with values of existing cfg
    """
    ...
    with open(file_name) as in_file:
        for line in in_file.readlines():
            ...
            if " = " in line:
                ...
                if v is None:
                    v = ''
                if k.strip in tags_keep:
                    result[k] = v
    return result

def collect_tags(tags_dic, _tags=TAGS, tags_keep=TAGS_KEEP):
    """
    [summary]
    ...
    """
    result = {}
    existing_tags = collect_existing(OUT_FILE, tags_keep)
...
def _make_base_config_file(tags):
    ...
    db_name = ""
    data_dir = ""
    # make sure that ~/.odoorc does not exist
    if os.path.exists(os.path.expanduser("~/.odoorc")):
        os.unlink(os.path.expanduser("~/.odoorc"))
    cmd = [
        "bin/odoo_bin",
        "-s",
        "-c",
    ...
    """
    we now create the real cfg file
    we have a dict base_tags, that contains all the tags to be written to the config fiele
        it has been creating by calling odoo, to generate a template config file by executing:
            bin/odoo ... -s etc/tmp.cfg
        This template is then "augmented" by the values in login_info.cfg.in, which has be created
        Then the projects site description has been processed.
    first process all the "main" tags, we have listed in tags and set their values
    then all the other tags that have bee created by odoo using the -s option
    """
