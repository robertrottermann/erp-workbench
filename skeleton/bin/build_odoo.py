...
def main(opts):  # nosetup=False, onlysetup=False):
    ...
    if not is_enterprise:
        ...
        cmd = [
            ...
            "1",
            "--branch",
            "%%s%%s" %% (VERSION, ODOO_MINOR),
            "--single-branch",
            "odoo%%s-server" %% VERSION,
        ]
        if not os.path.exists("odoo%%s-server" %% VERSION):
            p = Popen(cmd, stdout=PIPE)
            p.communicate()
        cmd = [
            "git",
            ...
            "%%s%%s" %% (VERSION, ODOO_MINOR),
            "--single-branch",
            "enterprise%%s" %% VERSION,
        ]
        if not os.path.exists("enterprise%%s" %% VERSION):
            p = Popen(cmd, stdout=PIPE)
            p.communicate()
    ...
    if is_venv():
        ...
        if not os.path.exists("odoo_bin"):
            if not is_enterprise:
                ...
                odoo_cmd = "../odoo%%s-server/odoo-bin" %% VERSION
    ...
    if is_enterprise:
        cmd = [pip_cmd, "install", "-r", "odoo%%s-server/requirements.txt" %% VERSION]
        p = Popen(cmd, stdout=PIPE)
        p.communicate()
    if python_cmd:
