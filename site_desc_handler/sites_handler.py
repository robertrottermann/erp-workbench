...
class SitesHandler(SiteDescHandlerMixin):
    ...
    def _create_sites_rep(self, running_path, force=None):
        ...
        if not os.path.exists(p1) or must_create or force:
            ...
            __ini__data = open(
                "%s/sites_list__init__.py" % templates.__path__[0]
            ).read()
            print_message = True
            if not os.path.exists(global_dir) or force:
                ...
                if is_localhost:
                    open("%s/sites_global/demo_global.py" % p1, "w").write(
                        SITES_GLOBAL_TEMPLATE % ("demo_global", template % defaults)
                    )
            else:
                print_message = False
    ...
    def check_and_create_sites_repo(self, force=False, must_update_ini=False):
        ...
        if not force:
            try:
                ...
                print("*" * 80)
                print(
                    "Due to changes in the site strucure, the sites list was reconstructed. Please restart"
                )
                print(bcolors.ENDC)
        ...
        for sitelist_name, sites_list_url in list(siteinfos.items()):
            ...
            if sites_list_url == "localhost":
                ...
                if p.returncode:
                    ...
                    print(
                        "please check if the sites_list in config/config.yaml is properly formated"
                    )
    ...
    def drop_site(self, template_name):
        ...
        if template_name in list(SITES_L.keys()):
            ...
            try:
                os.unlink(
                    os.path.normpath(
                        "%s/%s/sites_local/%s.py"
                        % (sites_list_path, origin, template_name)
                    )
                )
                os.unlink(
                    os.path.normpath(
                        "%s/%s/sites_local/%s.pyc"
                        % (sites_list_path, origin, template_name)
                    )
                )
            except:
                ...
                try:
                    if origin == "localhost":
                        os.unlink(
                            os.path.normpath(
                                "%s/%s/sites_global/%s.py"
                                % (sites_list_path, origin, template_name)
                            )
                        )
                        os.unlink(
                            os.path.normpath(
                                "%s/%s/sites_global/%s.pyc"
                                % (sites_list_path, origin, template_name)
                            )
                        )
                except:
    ...
    def find_template(self, template_name):
        ...
        if template_name in list(SITES.keys()):
            return open(
                "%ssites_global/%s.py" % (sites_list_path, template_name)
            ).read()
    ...
    def add_site_global(
        self, handler, template_name="", preset_values={}, sublist="localhost"
    ):
    ...
    def _add_site(self, where, template, no_outer, sublist):
        ...
        if where == "G":
            ...
            f_path = "%s/%s/sites_global/%s.py" % (
                self.handler.base_info["sitesinfo_path"],
                sublist,
                site_name,
            )
        elif where == "L":
            site_name = self.handler.site_name
            f_path = "%s/%s/sites_local/%s.py" % (
                self.handler.base_info["sitesinfo_path"],
                sublist,
                site_name,
            )
        ...
        return {"type": where, "site_info": outer_template}
    ...
    def check_pull(self, auto="check", opts=None):
        ...
        if folders and not opts.quiet:
            ...
            print("*" * 80)
            print(
                "pulling sites list (can be switched of runnning bin/e -c, option sites_autopull)"
            )
