...
class SiteDescHandlerMixin(PropertiesMixin):
    ...
    # when setting the passwords, we are also reading the values set in
    # the site description file
    def set_passwords_and_site_values(self, running_site):
        ...
        if 1:  #'site_name' in running_site.keys():
            ...
            site_name = running_site.get(
                "site_name", self.opts.name and self.opts.name.split(":")[0] or ""
            )
            ...
            self._db_user = self.opts.__dict__.get("db_user", db_user)

            db_user_pw = self.base_info.get("db_user_pw")
            self._db_user_pw = self.opts.__dict__.get("db_user_pw", db_user_pw)

            self._rpc_user = self.opts.__dict__.get("rpc_user", db_user)

            self._rpc_user_pw = self.opts.__dict__.get("rpc_password", db_user_pw)

            self._rpc_port = self.opts.__dict__.get("rpc_port", "8069")

            self._rpc_host = self.opts.__dict__.get("rpc_host", "localhost")

            # ----------
            # docker
            # ----------
            # docker_db_user is aliased to db_user
            self._docker_db_user = (
                self.subparser_name == "docker" and self.opts.docker_db_user
            ) or self.docker_defaults.get("docker_db_user", "")
            self._docker_db_user_pw = (
                self.subparser_name == "docker" and self.opts.docker_db_user_pw
            ...
            if not docker_rpc_user_pw:
                ...
                if not docker_rpc_user_pw:
                    docker_rpc_user_pw = self.docker_defaults.get(
                        "docker_rpc_user_pw", ""
                    )
            ...
            if not db_container_name:
                db_container_name = self.docker_defaults.get(
...
>>>>>>> docker_2
                if not docker_db_container:
                    try:
                        ...
                        print("*" * 80)
                        print(
                            "either the db container was missing and could not be created or some other problem"
                        )
                        print(str(e))
                        sys.exit()
            ...
            # ------------------------------------
            # version
            erp_version = running_site.get(
                "erp_version", running_site.get("odoo_version")
            )
            if not erp_version:
            ...
            # enterprise version
            self._is_enterprise = running_site.get("enterprise", "")
            ...
            # docker
            self._docker_base_image = docker.get(
                "base_image", "camptocamp/odoo-project:%s-latest" % self.erp_version
            )
            erp_image_version = docker.get(
                "erp_image_version", docker.get("odoo_image_version")
            )
            if not erp_image_version:
                erp_image_version = self.docker_defaults.get(
            ...
            self._docker_rpc_port = docker.get("erp_port", docker.get("odoo_port"))
            docker_long_polling_port = docker.get(
                "erp_longpoll", docker.get("odoo_longpoll")
            )
            ...
            self._docker_external_user_group_id = docker.get(
            ...
            # docker db
            # ------------------------------------
            self._docker_db_container_name = self.docker_defaults.get(
                "docker_db_container_name", ""
            )

            if self.subparser_name == "docker":
                self._docker_db_user = (
                    self.opts.docker_db_user
                    or self.docker_defaults.get("docker_db_user", "")
                )
            else:
    ...
    def handle_addons(self):
        ...
        _construct_sa(self.site_name, deepcopy(site_addons), self.site_skip_list)
    ...
    def prepare_properties(self, running_site):
        ...
        """
        self.set_passwords_and_site_values(running_site)
    ...
    def construct_defaults(self):
        ...
        if self.site.get("odoo_version"):
            ...
            print("*" * 80)
            print(
                'the site %s uses "odoo_version" but should use "erp_version", please fix'
                % self.site_name
            )
            print(bcolors.ENDC)
        ...
        # # are we dealing with an enterprise version
        default_values["is_enterprise"] = self.is_enterprise
        ...
        self.default_values["docker_port"] = self.docker_defaults.get(
            "docker_port", 9000
        )
        self.default_values["docker_long_poll_port"] = self.docker_defaults.get(
        ...
        self.default_values["log_db_level"] = self.docker_defaults.get(
            "docker_log_db", False
        )
    ...
    def XX_create_login_info(self, login_info):
        ...
        # -----------------
        # actual user
        if self.opts.__dict__.get("delete_site_local") or self.opts.__dict__.get(
            "drop_site"
        ):
            return
        if self.site:
            p = "%s/sites_global/%s.py" % (
                self.base_info["sitesinfo_path"],
                self.site_name,
            )
            if not self.site.get("remote_server"):
                print(SITE_HAS_NO_REMOTE_INFO % (self.site_name, os.path.normpath(p)))
            ...
            if self.site and not self.remote_servers.get(site_server_ip):
                ...
                if must_exit:
                    print(
                        SITE_UNKNOW_IP
                        % (site_server_ip, self.site_name, self.user, site_server_ip)
                    )
                    sys.exit()
