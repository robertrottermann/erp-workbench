...
if wkhtmltopdf:
    BASE_CLEAN_COMMANDS_PH1.append(
        "UPDATE ir_config_parameter SET value='%s' where key='webkit_path'"
        % wkhtmltopdf
    )
...
class OdooHandler(object):
    ...
    def dump(self, limit=10000):
        ...
        attachments = ir_atts.search([("db_datas", "!=", False)], limit=limit)
        ...
        for counter, a_id in enumerate(attachments, start=1):
            ...
            if attachment_data["res_model"] and not self.env.registry.get(
                ...
                logging.warning(
                    "not moving attachment %d because it links to unknown " "model %s",
                    attachment.id,
                    attachment_data["res_model"],
                )
                continue
            try:
                attachment.write({"datas": attachment_data["datas"], "db_datas": False})
            except Exception:
            ...
            if not counter % (len(attachments) / 100 or limit):
                ...
                line = (
                    "name: %(fname)s, %(dname)s, store: %(store_fname)s, size: %(size)s\n"
                    % data
                )
