...
try:
    ...
    print(
        bcolors.WARNING
        + bcolors.FAIL
        + "please run bin/pip install -r install/requirements.txt"
        + bcolors.ENDC
    )
...
def main():
    ...
    parser.add_argument(
        "-r", "--remote-host", action="store", dest="remote_host", help="remote host"
    )
    parser.add_argument(
        "-ru",
        "--remote-user",
    ...
    if len(unknownargs) != 2:
        print(
            bcolors.WARNING
            + "need exactly two arguments: soucedirectory, targetdirectory"
            + bcolors.ENDC
        )
        return
    ...
    if args.remote_host:
        ...
        target_d = os.path.normpath(
            "%s@%s:%s%s/dump/%s.dmp" % (ru, rh, home, target, target_name)
        )
        target_f = os.path.normpath(
            "%s@%s:%s%s/filestore/%s" % (ru, rh, home, target, target_name)
        )
        cmd_lines = [
            "rsync -av %s  %s" % (source_d, target_d),
            "rsync -av --delete %s/  %s/" % (source_f, target_f),
        ...
        for cmd_line in cmd_lines:
            ...
            print(cmd_line)
            p = subprocess.Popen(
                cmd_line, stdout=PIPE, env=dict(os.environ, PATH="/usr/bin"), shell=True
            )
            if args.verbose:
