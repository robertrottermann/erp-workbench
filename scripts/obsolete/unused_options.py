def parse_args():
    ...
    usage = (
        "create_system.py is a tool to create and maintain local odoo developement environment\n"
        "**************************\n"
    ...
    parser.add_argument(
        "--help", action=_HelpAction, help="help for help if you need some help"
    )  # add custom help
    ...
    parser_rpc.add_argument(
        "-SOSL",
        "--set-odoo-settings-local",
        action="store_true",
        dest="set_odoo_settings_local",
        default=False,
    ...
    parser_manage.add_argument(
        "-uiss",
        "--update-install-serversettings",
        action="store_true",
        dest="update_install_serversetting",
        default=False,
    ...
    parser_support = parser_s.add_parser(
        "support",
        help="the option -sites --support has the following subcommands",
    ...
    parser_docker.add_argument(
        ...
        action="store_true",
        dest="docker_create_update_container",
        default=False,
    ...
    parser_docker.add_argument(
        "-dSL",
        "--set-local-data-docker",
        action="store_true",
        dest="set_local_data_docker",
        default=False,
        help="force setting local data from the site description, even when we are on a remote site",
    )
    parser_docker.add_argument(
        "-dSOS",
        "--set-odoo-settings-docker",
        action="store_true",
        dest="set_odoo_settings_docker",
        default=False,
        help="set odoo settings like the mail handlers. The script tries to define for what ip",
    )
    parser_docker.add_argument(
    ...
    parser_docker.add_argument(
        "-dbiC",
        "--build_image_collect_sites",
        action="store_true",
        dest="use_collect_sites",
    ...
    parser_docker.add_argument(
        "-dcapw",
        "--docker-copy-admin-pw",
        action="store",
        dest="docker_copy_admin_pw",
    ...
    parser_docker.add_argument(
        ...
        "--recreate-container",
        action="store_true",
        dest="docker_recreate_container",
        default=False,
    ...
    parser_docker.add_argument(
        ...
        "--dockerdbname",
        action="store",
        dest="dockerdbname",  # no default, otherwise we can not get it from the site description
        help="user to access db in a docker, if not set, it is taken form the sites odoo stanza, default %s"
        % DOCKER_DEFAULTS["dockerdb_container_name"],
    )
    ...
    parser_docker.add_argument(
        ...
        "--dockerdbuser",
        action="store",
        dest="dockerdbuser",  # no default, otherwise we can not get it from the site description
        help="user to access db in a docker, if not set, it is taken form the sites odoo stanza, default %s"
        % DOCKER_DEFAULTS["dockerdbuser"],
    )

    parser_docker.add_argument(
        "-ddbpw",
        action="store",
        dest="dockerdbpw",  # no default, otherwise we can not get it from the site description
        help="password to access db in a docker, if not set, it is taken form the sites odoo stanza, default %s"
        % DOCKER_DEFAULTS["dockerdbpw"],
    )

    parser_docker.add_argument(
        "-drpcuser",
        action="store",
        dest="drpcuser",  # no default, otherwise we can not get it from the site description
        help="password to access db in a docker, if not set, it is taken form the sites odoo stanza, default %s"
        % DOCKER_DEFAULTS["dockerrpcuser"],
    )

    parser_docker.add_argument(
        "-drpcuserpw",
        action="store",
        dest="drpcuserpw",  # no default, otherwise we can not get it from the site description
        help="password to access db in a docker, if not set, it is taken form the sites odoo stanza, default %s"
        % DOCKER_DEFAULTS["dockerrpcuserpw"],
    )
