...
class MailHandler(InitHandler):
    ...
    def __init__(self, opts, sites=SITES):
        ...
        email_engine = create_engine(
            "mysql://%(user)s:%(passwd)s@%(host)s:%(port)s/%(db)s" % connect_dic
        )
    ...
    def get_domain_froxlor(self, name, customer_id):
        ...
        session = self.get_session()
        result = session.query(df).filter(
            and_(df.domain == name, df.customerid == customer_id)
        )
    ...
    def check_mail_user_froxlor(self, mail_user, server_data):
        ...
        if mail_user.customerid != server_data.get("customer_id"):
            ...
            raise EmailValueError(
                "email %s allready exists, but is managed by customer %s"
    ...
    def get_mail_user(self, email, server_data={}):
        ...
        result = session.query(self.mu).filter(
            or_(self.mu.email == email, self.mu.email_email == email)
        )
        ...
        if (
            mail_user
            and server_data
            and self.check_mail_user_froxlor(mail_user, server_data)
        ):
    ...
    def get_mail_virtual(self, email, server_data={}):
        ...
        if (
            mail_virtual
            and server_data
            and self.check_mail_virtual_froxlor(mail_virtual, server_data)
        ):
    ...
    def manage_mail_account(
        ...
        # - redirerect
        # - catch all
        result = {"mail_virtual": None, "mail_user": None}
        if cmd not in ["add", "delete", "update", "check"]:
            raise EmailValueError(
                "%s is not a vallid manage_mail_account command" % cmd
            )
        if cmd == "check":
            ...
            actual = self.manage_mail_account(
                mailuser, user_data, server_data, cmd="check"
            )
    ...
    def manage_mail(self):
        ...
        domain = self.get_domain_froxlor(
            mail_server_data.get("domain"), customer.customerid
        )
        if not domain:
            domain = self.create_domain_froxlor(
                mail_server_data.get("domain"), customer.customerid
            )

        # second step is to create and check all the email needed
        server_data = {"customer_id": customer.customerid, "domain_id": domain.id}
        missing_emails = {}
        bad_emails = {}
        existing_emails = {}
        ...
        for mailuser, user_data in list(mail_data.get("mail_user", {}).items()):
            result = self.manage_mail_account(
                mailuser, user_data, server_data, cmd="check"
            )

        new_rec = {"catch_all": catch_all}
