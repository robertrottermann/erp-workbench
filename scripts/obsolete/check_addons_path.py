...
def check_addons_paths(cfg_path, path_mapping=[], verbose=False, create=False):
    ...
    addons = [
        ...
        for a in [
            l.split("=")[-1].strip()
            for l in open(cfg_path).read().split("\n")
            if l.find("addons_path") > -1
        ][0].split(",")
    ]
    result = []
    for addon in addons:
        ...
        if not os.path.exists(addon):
            ...
            if create:
                try:
                    ...
                    result.append(
                        bcolors.FAIL + "%s could not be created" % addon + bcolors.ENDC
                    )
    if result:
        print("\n".join(result))
...
if __name__ == "__main__":
    ...
    check_addons_paths(
        "/home/robert/erp_workbench/afbstest/etc/openerp-server.conf",
        verbose=True,
