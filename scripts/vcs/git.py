...
class GitRepo(BaseRepo):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if depth is not None and depth != "None":
            ...
            # of a global setting
            invalid = UserError(
                "Invalid depth value %r for Git repository "
    ...
    def git_version(self):
        ...
        return cls.init_git_version(utils.check_output(["git", "--version"]))
    ...
    def init_git_version(cls, v_str):
        ...
        try:
            ...
            raise ValueError(
                "Could not parse git version output %r. Please " "report this" % v_str
            )
        return version

    def log_call(
        self, cmd, callwith=subprocess.check_call, log_level=logging.INFO, **kw
    ):
    ...
    def uncommitted_changes(self):
        ...
        with working_directory_keeper:
            os.chdir(self.target_dir)
            p = subprocess.Popen(
                ["git", "status", "--short"], stdout=subprocess.PIPE, env=SUBPROCESS_ENV
            )
            out = p.communicate()[0]
    ...
    def get_current_remote_fetch(self):
        with working_directory_keeper:
            os.chdir(self.target_dir)
            for line in self.log_call(
                ["git", "remote", "-v"], callwith=check_output
            ).splitlines():
    ...
    def offline_update(self, revision):
        ...
        if current_url != self.url:
            raise UserError(
                "Existing Git repository at %r fetches from %r "
    ...
    def fetch_remote_sha(self, sha):
        ...
        """
        logger.warn(
            "%s: pointing to a remote commit directly by its SHA "
    ...
    def get_update(self, revision):
        ...
        if revision in self.dangerous_revisions:
            logger.warn(
                "%s> use of %r as revision in the recipe may "
        ...
        with working_directory_keeper:
            ...
            os.chdir(target_dir)
            self.log_call(
                ["git", "remote", "add" if is_new else "set-url", BUILDOUT_ORIGIN, url],
            ...
            if rtype == "tag":
                fetch_refspec = "+refs/tags/%s:refs/tags/%s" % (revision, revision)
            else:
            ...
            if rtype == "tag":
                ...
                raise NotImplementedError("Unknown remote reference type %r" % rtype)
    ...
    def update_fetched_branch(self, branch):
        ...
        self.log_call(
            [
                "git",
                "update-ref",
                "/".join(("refs", "remotes", BUILDOUT_ORIGIN, branch)),
                "FETCH_HEAD",
            ]
        )
        if self.options.get("depth") or branch == "HEAD":
            ...
            # not on Git 1.7
            self.log_call(["git", "checkout", "FETCH_HEAD"], callwith=update_check_call)
            if branch != "HEAD":
                self.log_call(
                    ["git", "branch", "-f", branch], callwith=update_check_call
                )
            return
    ...
    def merge(self, revision):
        ...
        with working_directory_keeper:
            if not self.is_versioned(self.target_dir):
                raise RuntimeError(
                    "Cannot merge into non existent "
                    "or non git local directory %s" % self.target_dir
                )
            os.chdir(self.target_dir)
    ...
    def archive(self, target_path):
        ...
        with working_directory_keeper:
            ...
            subprocess.check_call(["git", "archive", revision, "-o", target_tar.name])
            subprocess.check_call(
                ["tar", "-x", "-f", target_tar.name, "-C", target_path]
            )
    ...
    def revert(self, revision):
        with working_directory_keeper:
            ...
            if self._is_a_branch(revision):
                self.log_call(
                    ["git", "reset", "--hard", BUILDOUT_ORIGIN + "/" + revision],
                    callwith=update_check_call,
                )
            else:
