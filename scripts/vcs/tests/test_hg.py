...
class HgBaseTestCase(VcsTestCase):
    ...
    def get_parent_node(self, repo):
        ...
        p = subprocess.Popen(
            ["hg", "--cwd", repo, "parents", "--template={node} "],
    ...
    def create_src(self):
        ...
        f.close()
        subprocess.call(
            ["hg", "commit", "-A", "-m", "initial commit", "-u", COMMIT_USER_FULL]
        )
        ...
        f.close()
        subprocess.call(["hg", "commit", "-m", "in branch", "-u", COMMIT_USER_FULL])
    ...
    def assertRevision(self, branch, revno):
        p = subprocess.Popen(
            ["hg", "--cwd", branch.target_dir, "parents", "--template={rev}\n"],
            stdout=subprocess.PIPE,
        )
...
class HgTestCase(HgBaseTestCase):
    ...
    def test_hgrc_paths_update(self):
        ...
        self.assertEqual(parser.get("paths", "buildout_save_1"), self.src_repo)
    ...
    def test_failed(self):
        ...
        self.assertRaises(subprocess.CalledProcessError, repo.get_update, "default")
...
class HgOfflineTestCase(HgBaseTestCase):
    ...
    def test_update_fixed_rev_by_tag(self):
        ...
        repo = self.make_clone("clone to update", "default")
        subprocess.check_call(
            ["hg", "--cwd", repo.target_dir, "tag", "-r", self.rev0, "first"]
        )
        repo("first")
        self.assertRevision(repo, 0)
    ...
    def test_update_missing_tag(self):
        ...
        repo = self.make_clone("clone to update", "default")
        subprocess.check_call(
            ["hg", "--cwd", self.src_repo, "tag", "-r", self.rev0, "in-src-only"]
        )
        repo(self.rev0)
