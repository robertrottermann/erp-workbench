...
class GitBaseTestCase(VcsTestCase):
    ...
    def create_src(self):
        ...
        self.commit_1_sha = git_write_commit(
            self.src_repo, "tracked", "first", msg="initial commit"
        )
        self.commit_2_sha = git_write_commit(
            self.src_repo, "tracked", "last", msg="last commit"
        )
...
class GitTestCase(GitBaseTestCase):
    def test_init_depth(self):
        ...
        self.assertRaises(UserError, GitRepo, "/some/target", self.src_repo, depth="A")
        self.assertRaises(UserError, GitRepo, "/some/target", self.src_repo, depth="-1")
    ...
    def test_clean(self):
        ...
        try:
            ...
            self.fail("clean() should not fail if " "clone not already done")
        ...
        os.mkdir(dirty_dir)
        dirty_files = (
            os.path.join(repo.target_dir, "untracked"),
        ...
        for path in dirty_files:
            self.assertFalse(
                os.path.exists(path), "Untracked file should have been removed"
            )
        self.assertFalse(
            os.path.exists(dirty_dir), "Untracked dir should have been removed"
        )
    ...
    def test_clone_on_sha_update(self):
        ...
        new_sha = git_write_commit(
            self.src_repo, "tracked", "new contents", msg="new commit"
        )
        repo(new_sha)
    ...
    def test_update_needs_pull(self, depth=None):
        ...
        new_sha = git_write_commit(
            self.src_repo, "tracked", "new content", msg="new commit"
        )
    ...
    def test_update_no_ff(self):
        ...
        new_sha2 = git_write_commit(
            self.src_repo, "tracked", "back to normal", msg="regular commit"
        )
        repo("master")
    ...
    def test_query_unknown_remote_ref(self):
        ...
        repo = GitRepo(target_dir, self.src_repo)("master")
        self.assertEqual(
            repo.query_remote_ref(BUILDOUT_ORIGIN, "master"),
            ("branch", self.commit_2_sha),
        )
        self.assertEqual(
            repo.query_remote_ref(BUILDOUT_ORIGIN, "deadbeef"), (None, "deadbeef")
        )
    ...
    def test_clone_remote_HEAD(self):
        ...
        repo = GitRepo(target_dir, self.src_repo)
        subprocess.check_call(["git", "checkout", self.commit_1_sha], cwd=self.src_repo)
        repo("HEAD")
        self.assertEqual(repo.parents(), [self.commit_1_sha])
...
class GitBranchTestCase(GitBaseTestCase):
    ...
    def test_switch_local_branch(self):
        ...
        branch("master")
        git_write_commit(target_dir, "tracked", "last after branch", msg="last version")
        ...
        git_write_commit(target_dir, "tracked", "last after branch", msg="last version")
        branch("somebranch")
        git_write_commit(target_dir, "tracked", "last from branch", msg="last version")
    ...
    def test_switch_local_branch_depth(self):
        ...
        branch("master")
        git_write_commit(target_dir, "tracked", "last after branch", msg="last after")

        # commit in the remote branch
        git_write_commit(
            self.src_repo, "tracked", "new in branch", msg="last from branch"
        )
    ...
    def test_switch_remote_branch(self, depth=None):
        ...
        git_write_commit(
            self.src_repo, "tracked", "last after remote branch", msg="last version"
        )
    ...
    def test_clone_on_branch(self, depth=None):
        ...
        sha = git_write_commit(
            self.src_repo, "tracked", "in branch", msg="last version"
        )
        repo("somebranch")
    ...
    def test_clone_on_branch_update_HEAD(self):
        ...
        sha_master = git_write_commit(
            self.src_repo, "tracked", "after branch", msg="new commit on master"
        )
    ...
    def test_sha_pinning_branch_indication(self):
        ...
        target_dir = os.path.join(self.dst_dir, "to_branch")
        sha = git_write_commit(
            self.src_repo, "tracked", "in branch", msg="last in branch"
        )
        # rewinding src repo to master branch so that sha is unreachable
        # from HEAD
        subprocess.check_call(["git", "checkout", "master"], cwd=self.src_repo)
        sha_master = git_write_commit(
            self.src_repo, "tracked", "on master", msg="new in master"
        )
        ...
        self.assertEqual(repo.parents(), [sha])
        self.assertNotEqual(
            subprocess.call(["git", "cat-file", "-e", sha_master], cwd=target_dir),
            0,
            msg="SHA should not have been fetched",
        )
...
class GitMergeTestCase(GitBaseTestCase):
    def create_src(self):
        ...
        git_write_commit(
            self.src_repo, "file_on_branch1", "file on branch 1", msg="on branch 1"
        )
        ...
        git_write_commit(
            self.src_repo, "file_on_branch2", "file on branch 2", msg="on branch 2"
        )
        self.checkout_branch(self.src_repo, "master")
    ...
    def test_01_check_src_repo(self):
        ...
        repo("master")
        self.assertFalse(
            os.path.exists(os.path.join(target_dir, "file_on_branch1")),
            "file_on_branch1 should not exist",
        )
        self.assertFalse(
            os.path.exists(os.path.join(target_dir, "file_on_branch2")),
...
class GitTagTestCase(GitBaseTestCase):
    ...
    def test_query_remote(self):
        ...
        with working_directory_keeper:
            ...
            os.chdir(target_dir)
            subprocess.check_call(["git", "remote", "add", "orig", self.src_repo])
    ...
    def test_revert_to_tag(self):
        ...
        subprocess.check_call(
            ["git", "fetch", BUILDOUT_ORIGIN, "+refs/tags/sometag:refs/tags/sometag"],
            cwd=target_dir,
        )
        repo.revert("sometag")
        self.assertEqual(repo.parents(), [self.commit_1_sha])
    ...
    def test_update_to_tag(self, depth=None):
        ...
        repo("sometag")
        self.assertEqual(repo.parents(), [self.commit_1_sha])
        subprocess.check_call(["git", "checkout", "sometag"], cwd=repo.target_dir)
    ...
    def test_update_tag_to_head(self, depth=None):
        ...
        subprocess.check_call(["git", "checkout", "sometag"], cwd=repo.target_dir)
        self.assertEqual(
            check_output(["git", "tag"], cwd=repo.target_dir).strip(), "sometag"
        )
        repo("master")
        self.assertEqual(repo.parents(), [self.commit_2_sha])
...
class GitAnnotatedTagTestCase(GitTagTestCase):
    ...
    def make_tag(self, tag):
        subprocess.check_call(
            ["git", "tag", "-a", "-m", "Annotation", tag, self.commit_1_sha]
        )
