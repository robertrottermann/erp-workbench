...
class BzrTestCase(BzrBaseTestCase):
    ...
    def test_parents_revid(self):
        ...
        self.assertTrue(
            parents[0].startswith("revid:test@example.org-"),
    ...
    def test_branch_options_conflict(self):
        ...
        branch = BzrBranch(
            target_dir,
            self.src_repo,
            **{"bzr-init": "branch", "bzr-stacked-branches": "True"}
        )
        self.assertRaises(Exception, branch, "last:1")

    def test_branch_bzr_init(self):
        target_dir = os.path.join(self.dst_dir, "My branch")
        branch = BzrBranch(target_dir, self.src_repo, **{"bzr-init": "branch"})
        branch("last:1")
        self.assertRevision2(branch)
    ...
    def test_branch_stacked_deprecated(self):
        target_dir = os.path.join(self.dst_dir, "My branch")
        branch = BzrBranch(
            target_dir, self.src_repo, **{"bzr-stacked-branches": "True"}
        )
        branch("last:1")
        self.assertRevision2(branch)
    ...
    def test_branch_stacked(self):
        target_dir = os.path.join(self.dst_dir, "My branch")
        branch = BzrBranch(target_dir, self.src_repo, **{"bzr-init": "stacked-branch"})
        branch("last:1")
        self.assertRevision2(branch)
    ...
    def test_checkout_lightweight(self):
        target_dir = os.path.join(self.dst_dir, "My branch")
        branch = BzrBranch(
            target_dir, self.src_repo, **{"bzr-init": "lightweight-checkout"}
        )
        branch("1")
    ...
    def test_clean(self):
        ...
        try:
            ...
            self.fail("clean() should not fail if " "branch not already done")
        ...
        with working_directory_keeper:
            ...
            bzr_status = subprocess.Popen(["bzr", "status"], stdout=subprocess.PIPE)
    ...
    def test_url_update(self):
        ...
        self.assertEqual(
            branch.parse_conf(),
            dict(buildout_save_parent_location_1=old_src, parent_location=new_src),
        )
    ...
    def test_lp_url_nobzrlib(self):
        ...
        save = vcs.bzr.LPDIR
        vcs.bzr.LPDIR = None
        self.assertRaises(RuntimeError, BzrBranch, "", "lp:anybox.recipe.openerp")
        vcs.bzr.LPDIR = save
    ...
    def test_failed(self):
        ...
        self.assertRaises(subprocess.CalledProcessError, branch.get_update, "default")
    ...
    def test_merge(self):
        ...
        BzrBranch(target_dir, self.src_repo)("last:1")
        BzrBranch(target_dir, to_merge, **{"bzr-init": "merge"})("last:1")
        os.chdir(current)
...
class BzrOfflineTestCase(BzrBaseTestCase):
    ...
    def test_update_available_revno_url_change(self):
        ...
        new_branch = BzrBranch(
            branch.target_dir, "http://other.url.example", offline=True
        )
    ...
    def test_update_available_tag_is_local_fixed_revision(self):
        ...
        subprocess.check_call(
            ["bzr", "tag", "-r", "1", "sometag"], cwd=branch.target_dir
        )
        branch("tag:sometag")
        self.assertRevision1(branch)
    ...
    def test_lightweight_checkout_noupdate(self):
        ...
        branch = self.make_local_branch(
            "clone to update", "1", options={"bzr-init": "lightweight-checkout"}
        )
    ...
    def test_lightweight_checkout_noupdate_fixed_rev(self):
        """[offline mode] lightweight checkouts shall not be updated."""
        branch = self.make_local_branch(
            "clone to update", "last:1", options={"bzr-init": "lightweight-checkout"}
        )
