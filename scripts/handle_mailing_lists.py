...
def main(opts):
    ...
    cursor.execute(
        "select id from mail_mass_mailing_list where name = '%s'" % mlist_name
    )
    row = cursor.fetchone()
    if not row:
        print("*" * 80)
        print("%s mailing list does not exist for %s" % (mlist_name, opts.dbname))
        sys.exit()
    ...
    for line in emails:
        if line.find("@") > -1:
            ...
            email = line.strip()
            s = (
                "select id from mail_mass_mailing_contact where email = '%s' and list_id = %s"
            ...
            if not row:
                s = (
                    "INSERT INTO mail_mass_mailing_contact (email, list_id) values ('%s', %s)"
                    % (email, mailinglist_id)
                )
                cursor.execute(s)
                conn.commit()
