...
try:
    ...
    import sys
    from messages import MODULE_MISSING

    print(MODULE_MISSING % "colored")
    sys.exit()
...
def searchRepositories():
    ...
    for curdir in dirs:
        ...
        for directory, dirnames, filenames in os.walk(curdir):
            ...
            if argopts.get("depth", None) is None or level <= argopts.get(
                "depth", None
            ):
                if ".git" in dirnames:
...
def checkRepository(rep, branch, return_counts={}):
    aitem = []
    mitem = []
    ditem = []
    ...
    html.topush = ""
    html.topull = ""
    return_counts["to_pull"] = {}
    return_counts["to_push"] = {}
    if branch != "":
        ...
        for r in remotes:
            ...
            if count > 0:
                ...
                html.topush += (
                    '<b style="color:black">%s</b>[<b style="color:blue">To Push:</b><b style="color:black">%s</b>]'
                    % (r, count)
                )
        ...
        for r in remotes:
            ...
            if count > 0:
                ...
                html.topull += (
                    '<b style="color:black">%s</b>[<b style="color:blue">To Pull:</b><b style="color:black">%s</b>]'
                    % (r, count)
                )
    if ischange or not argopts.get("quiet", False):
        ...
        if len(changes) > 0:
            ...
            html.strlocal += "To Commit:%s" % (lenFilesChnaged)
            strlocal += "]"
            html.strlocal += "]</b>"
        else:
            strlocal = ""
        ...
        if argopts.get("verbose", False):
            if ischange > 0:
                ...
                for c in changes:
                    ...
                    html.msg += (
                        '<li> <b style="color:orange">[To Commit] </b>%s</li>\n' % c[1]
                    )
                    if not argopts.get("email", False):
                        print(filename)
                html.msg += "</ul>\n"
                ...
                for r in remotes:
                    ...
                    if len(commits) > 0:
                        ...
                        for commit in commits:
                            ...
                            html.msg += (
                                '<li><b style="color:blue">[To Push] </b>%s</li>\n'
                                % commit
                            )
                            if not argopts.get("email", False):
                                print(pcommit)
                ...
                for r in remotes:
                    ...
                    if len(commits) > 0:
                        ...
                        for commit in commits:
                            ...
                            html.msg += (
                                '<li><b style="color:blue">[To Pull] </b>%s</li>\n'
                                % commit
                            )
                            if not argopts.get("email", False):
                                print(pcommit)
...
def getLocalToPush(rep, remote, branch):
    if not hasRemoteBranch(rep, remote, branch):
        return []
    result = gitExec(rep, "log %(remote)s/%(branch)s..%(branch)s --oneline" % locals())
...
def getRemoteToPull(rep, remote, branch):
    if not hasRemoteBranch(rep, remote, branch):
        return []
    result = gitExec(rep, "log %(branch)s..%(remote)s/%(branch)s --oneline" % locals())
...
def getDefaultBranch(rep):
    ...
    gitbranch = gitExec(rep, "branch" % locals())
...
def getRemoteRepositories(rep):
    result = gitExec(rep, "remote" % locals())
...
def gitcheck(return_counts={}):
    ...
    for r in repo:
        ...
        for b in branch:
            if checkRepository(r, b, return_counts):
                actionNeeded = True
...
def sendReport(content):
    ...
    text = (
        "Gitcheck report for %s created on %s\n\n This file can be seen in html only."
        % (html.path, html.timestamp)
    )
    htmlcontent = (
...
def usage():
    ...
    print(
        "  -d <dir>, --dir=<dir>                Search <dir> for repositories (can be used multiple times)"
    )
    print(
