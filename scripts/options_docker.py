...
def add_options_docker(parser, result_dic):
    ...
    parser_docker.add_argument(
        "-dSOS",
        "--set-erp-settings-docker",
        action="store_true",
        dest="set_erp_settings_docker",
        default=False,
    ...
    parser_docker.add_argument(
        ...
        help="user to access db in a docker, if not set, it is taken form the sites erp stanza, default %s"
        % DOCKER_DEFAULTS["docker_db_container_name"],
    )
    ...
    parser_docker.add_argument(
        ...
        "--dockerdbuser",
        action="store",
        dest="docker_db_user",  # no default, otherwise we can not get it from the site description
        help="user to access db in a docker, if not set, it is taken form the sites erp stanza, default %s"
        % DOCKER_DEFAULTS["docker_db_user"],
    )

    parser_docker.add_argument(
        "-ddbpw",
        action="store",
        dest="docker_db_user_pw",  # no default, otherwise we can not get it from the site description
        help="password to access db in a docker, if not set, it is taken form the sites erp stanza, default %s"
        % DOCKER_DEFAULTS["docker_db_user_pw"],
    )
    ...
    parser_docker.add_argument(
        ...
        help="password to access db in a docker, if not set, it is taken form the sites erp stanza, default %s"
        % DOCKER_DEFAULTS["docker_rpc_user"],
    )
    ...
    parser_docker.add_argument(
        ...
        help="password to access db in a docker, if not set, it is taken form the sites erp stanza, default %s"
        % DOCKER_DEFAULTS["docker_rpc_user_pw"],
    )
    ...
    parser_docker.add_argument(
        "-dI",
        "--dinstall_erp_modules",
        action="store_true",
        dest="dinstall_erp_modules",
        default=False,
